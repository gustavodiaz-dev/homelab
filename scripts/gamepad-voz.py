...
def main():
    print("[gamepad-voz] Iniciando...")
    while True:
        dev = find_controller()
        if not dev:
            print("[gamepad-voz] Control no encontrado, reintentando en 5s...")
            time.sleep(5)
            continue

        print(f"[gamepad-voz] Control: {dev.name} en {dev.path}")
        ...
        try:
            ...
            print("[gamepad-voz] Control desconectado, esperando...")
            time.sleep(3)
